...
void moosic::MusicEngine::insert(moosic::MusicQueue& queue)
{
	...
	for (int i = 0; i < (int)this->queue.get_queue().size(); i++)
	{
		...
		if (ma_sound_init_from_file(&engine, queue.get_queue()[i].path.c_str(), MA_SOUND_FLAG_ASYNC | MA_SOUND_FLAG_STREAM, 0, 0, &this->queue.get_queue()[i].sound) != MA_SUCCESS)
		{
			// TODO: Logging
...
{
	if (ma_sound_at_end(&queue.get_queue()[index].sound)) return moosic::MusicState::DONE;
	if (ma_sound_is_playing(&queue.get_queue()[index].sound)) return moosic::MusicState::PLAYING;
...
void moosic::MusicEngine::start_at(uint32_t id)
{
	restart();
	if (id <= queue.get_queue().size() && id >= 0) 
		index = id;
	else // TODO: Proper logging
