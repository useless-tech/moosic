...
int main(int, char **)
{
    ...
    std::string id = "x-xTttimcNk";

    moosic::YoutubeBackend youtube_backend(id);
            ...
            {
                {
                    ...
                    {
                        youtube_backend = moosic::YoutubeBackend(id);
                        song_name = youtube_backend.get_title();
                        artist = youtube_backend.get_artist();
                        cover = youtube_backend.get_album_cover();

                        texture = load_texture_from_image(renderer, cover.c_str());
                    ...
                    {
                        moosic::MusicInfo info;
                        info.name = youtube_backend.get_title();
                        info.path = std::filesystem::current_path().string() + "/" + youtube_backend.get_title() + " [" + id + "].mp3";

                        youtube_backend.get_file();
                        queue.push(info);
